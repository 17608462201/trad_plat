...
<table class="layui-table" lay-data="{height:550,url:'${ctx }/loan/getList', page:true, limit:10, id:'tables'}" lay-filter="tree_filter">
  <thead>
    <tr>
      ...
      <th lay-data="{field:'applyTime', width:110}">申请时间</th>
      <th lay-data="{fixed: 'right', width:70, toolbar: '#barDemo'}"></th>
    </tr>
  </thead>
</table>
	 ...
	 table.on('tool(tree_filter)', function(obj){
		    ...
		    if(obj.event === 'examine'){
		    	layer.open({
		            ...
		            ,content: '${ctx}/loan/loanExamine?loanId='+data.id
		            ,btn: ['确认', '关闭']
		            ,yes: function(){
		            	 var body = layer.getChildFrame('body', 0);
		            	 var loanStatus = body.find('select[name="loanStatus"]').val();
		            	 var loanOpinion = body.find('input[name="loanOpinion"]').val();
		            	 var loanId = body.find('input[name="loanId"]').val();
		            	 $.post("${ctx}/loan/upLoanStatus?loanId="+loanId+"&loanStatus="+loanStatus+"&loanOpinion="+loanOpinion,function(text){
			          		  if(text=='200'){
			          			layer.closeAll();
				          		  window.location.href ='${ctx }/loan/init';
			          		  }else{
